...
<div class="mx-auto max-w-3xl space-y-12 px-4 py-8 sm:py-16">
	<div class="content-block space-y-12 sm:space-y-20">
		...
		<section class="space-y-4">
			...
			<p class="max-w-xl text-lg leading-relaxed text-slate-400">
		...
		{#if data.latestPosts.length > 0}
			<section class="space-y-6">
				<div class="flex items-center justify-between">
					...
					<a
						href="/blog"
						class="flex items-center gap-1 text-sm text-slate-500 transition-colors hover:text-cyan-400"
					>
						All posts
						<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M5 12h14"/><path d="m12 5 7 7-7 7"/></svg>
					</a>
				...
				<div class="space-y-0">
					{#each data.latestPosts as post (post.id)}
						<a
							href="/blog/{post.slug}"
							class="group -mx-2 flex items-start justify-between gap-4 rounded-md border-b border-slate-800/60 px-2 py-4 transition-colors last:border-0 hover:bg-slate-800/20"
						>
							<div class="min-w-0 space-y-1">
								<p class="truncate font-medium transition-colors group-hover:text-cyan-400">
									{post.title}
								</p>
								{#if post.excerpt}
									<p class="line-clamp-1 text-sm text-slate-500">{post.excerpt}</p>
								{/if}
					...
					{#each data.featuredProjects as project (project.id)}
						...
						>
							{#if project.image}
								...
								/>
							{/if}
							<h3 class="font-semibold text-slate-200 transition-colors group-hover:text-cyan-400">
								{project.title}
							</h3>
