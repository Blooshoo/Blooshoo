...
Personal website and portfolio â€” retro pixel / late-90s aesthetic. Built with
...
| Auth | NextAuth v5 â€” credentials provider against `users` table |
...
| `/` | Home â€” hero with blinking cursor, latest 3 posts, featured projects (mine only) |
...
| `/capes` | ðŸ¦¸ Cape product page (easter egg â€” **not in nav**, discoverable via blog links). Sold-out listing with 3 product images, struck-out $49.99 price, waiting-list CTA â†’ `/contact` |

| `/contact` | Contact form with math challenge captcha and honeypot. Submissions saved to database. |



### Bloo panel (secret URL â€” not linked anywhere public)

| Route | Description |

|---|---|

| `/bloo` | Dashboard â€” stats for posts, projects, media |
...
| `/bloo/posts` | Blog CRUD list â€” Edit / Delete per row, "New Post" button |

| `/bloo/posts/new` | New post form â€” Tiptap rich text editor |

| `/bloo/posts/[id]` | Edit post |

| `/bloo/projects` | Project CRUD â€” admins see all, contributors see own |

| `/bloo/projects/new` | New project form â€” category, owner type, dynamic links |

| `/bloo/projects/[id]` | Edit project |

| `/bloo/users` | Admin-only user management â€” create, delete, reset passwords |
...
- **Blog content:** `.prose-bloo` scoped CSS â€” VT323 teal headings, `#00aaff`
...
| `posts` | id, title, slug (unique), content (HTML), excerpt, cover_image, status, tags (JSON), author_id (FKâ†’users), created_at, updated_at |

| `projects` | id, title, description, category (website/game/mod/other), links (JSON), image, owner_type (mine/friend), owner_name, owner_id (FKâ†’users), featured (bool), sort_order, created_at, updated_at |
...
- **Admin** â€” full access to all `/bloo` pages, manage users, set project featured

- **Contributor** â€” CRUD own posts/projects, no `/bloo/users`, no featured flag

- Session carries `id`, `displayName`, `role` â€” available via `auth()` everywhere

- Auth helpers: `requireAuth()` â†’ redirects to `/bloo/login`, `requireAdmin()` â†’
...
| `auth.config.ts` | Edge-safe config â€” JWT/session callbacks, `authorized` guard, `trustHost: true`. No Node.js-only imports (no `fs`, no `better-sqlite3`). Used by middleware. |

| `auth.ts` | Full NextAuth config â€” spreads `authConfig`, adds credentials provider with bcrypt + DB lookup. Exports `handlers`, `signIn`, `signOut`, `auth`. |

| `middleware.ts` | Runs `NextAuth(authConfig)` on Edge. Matcher: `["/bloo", "/bloo/((?!login).*)"]` â€” excludes `/bloo/login` to prevent redirect loops. |
...
- **Middleware** protects all `/bloo/*` except `/bloo/login` â€” unauthenticated

  requests to protected routes get a 307 to `/bloo/login?callbackUrl=...`

- **`app/bloo/layout.tsx`** checks `auth()` â€” if no session, renders children

  without the admin sidebar/header (the login page). If session exists, renders

  the full admin chrome.

- **`trustHost: true`** in `auth.config.ts` â€” required when running behind
...
# Remove ADMIN_PASSWORD from .env.local after â€” the hash is in the DB now.

```



### Option B: Sharing with a friend (pre-hashed â€” SAFE)
...
Your friend never sees your plaintext password â€” the hash is one-way bcrypt.
...
Log in as admin â†’ `/bloo/users` â†’ "Create User". Each user can then change
...
docker compose exec blooshoo npm run seed   # â† creates your admin user
...
`ADMIN_PASSWORD`, and `ADMIN_PASSWORD_HASH` from `.env.local` â€” they were only
...
| `AUTH_URL` | production | Full URL the site is served at â€” **must match the actual port** (e.g. `http://localhost:7777` if running on 7777) |
...
| `ADMIN_PASSWORD` | for seed (opt A) | Plaintext â€” remove after seeding |

| `ADMIN_PASSWORD_HASH` | for seed (opt B) | Pre-hashed â€” safe to share |
...
- [ ] Finish & re-enable Revenge scene (out of rotation â€” needs visual polish, files at `components/revenge-scene.ts`)
