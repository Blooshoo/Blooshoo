...
| Auth | better-auth — credentials provider against `users` table with bcrypt |
...
| Table | Key columns |
|---|---|
| `users` | id, username (unique), display_name, password_hash (bcrypt), role (admin/contributor), email, email_verified, image, created_at, updated_at |
| `session` | id, token (unique), user_id (FK→users), expires_at, ip_address, user_agent, created_at, updated_at |
| `account` | id, account_id, provider_id, user_id (FK→users), password, access_token, refresh_token, created_at, updated_at |
...
| File | Purpose |
|---|---|
| `lib/auth.ts` | better-auth config — Drizzle adapter pointed at existing SQLite DB, credentials provider with bcrypt, session config (30 days, cookie cache), user field mapping (`email` → `username`, `name` → `display_name`, etc.) |
| `middleware.ts` | Checks `auth.api.getSession()` to protect `/bloo/*` routes. Matcher: `["/bloo/:path*"]`. Redirects unauthenticated to `/bloo/login`, redirects logged-in users away from `/bloo/login`. |
| `app/api/auth/[...all]/route.ts` | Re-exports `{ GET, POST }` from `toNextJsHandler(auth)`. |
| `lib/auth-helpers.ts` | `requireAuth()` and `requireAdmin()` server helpers using `auth.api.getSession()`. |

### Key auth behaviors

- **Middleware** protects all `/bloo/*` except `/bloo/login` — unauthenticated
  requests get redirected to `/bloo/login`.
- **`app/bloo/layout.tsx`** checks session — if no session, renders children
  without the admin sidebar/header (the login page). If session exists, renders
  the full admin chrome.
- **Logged-in users** visiting `/bloo/login` get redirected to `/bloo`.
...
|---|---|---|
| `AUTH_SECRET` | yes | Auth secret — 32+ char string (`openssl rand -base64 32`). Replaces old `SESSION_SECRET`. |
| `BETTER_AUTH_URL` | yes (production) | Base URL of the app (e.g., `http://192.168.0.124:7777`) |
...
### 2026-05-07 — Auth Migration #3: iron-session → better-auth

Ripped out `iron-session` (credentials-based session library) and replaced it
with `better-auth` (full auth framework with Drizzle adapter).

- Added `account`, `session`, `verification` tables + `email`, `email_verified`,
  `image`, `updated_at` columns to `users`
- better-auth configured with Drizzle adapter pointed at existing SQLite DB,
  credentials provider using bcrypt, 30-day sessions with cookie cache
- User `email` field populated as `username@blooshoo.internal` to satisfy
  better-auths email format validation
- Middleware rewritten Edge-safe (cookie-only check, no DB import) after
  discovering Edge runtime cannot import `better-sqlite3`
- `requireAuth()` / `requireAdmin()` helpers preserved, now using
  `auth.api.getSession()`
- Seed script and create-user API both create matching `account` rows
- Docker: `AUTH_SECRET` replaces `SESSION_SECRET`, `BETTER_AUTH_URL` added
- 18 files touched, 3 deleted (`lib/session.ts`, old login/logout API routes)


**Total cost:** ~30 million tokens of extensive peer pressure. nbd. >_>
